...
class account_payment(models.Model):
	...
	def _compute_is_journal(self):
		for payment in self:
			payment.to_journal_id=payment.journal_id.id
			payment.from_journal_id = payment.destination_journal_id.id
	...
	def _prepare_move_line_default_vals(self, write_off_line_vals=None):
		...
		if self.is_internal_transfer == True and self.internal_transfer_type == 'j_to_j' :
			liquidity_line_account = self.journal_id.payment_credit_account_id.id
...
class account_move_line(models.Model):
	...
	journal_rec = fields.Many2one('account.journal',string="From Account",compute='_compute_journal')
	journal_name = fields.Integer(string="journ",store=True)

	def _compute_journal(self):
		move = self.env['account.move'].search([('id', '=', self.move_id.id)])
		print(move.journal_id.name)
		self.journal_rec=self.move_id.journal_id.id
		self.journal_name = self.journal_rec.id
