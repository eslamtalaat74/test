...
class BiReportAgedPartnerBalance(models.AbstractModel):
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length, branch):
        periods = {}
        ...
        if not branch:
            ...
            query = '''
                        ...
                        WHERE (l.account_id = account_account.id)
                            ...
                            AND (l.date <= %s)
                            AND l.company_id IN %s
                            AND am.branch_id in ''' + temp + '''
                        ORDER BY UPPER(res_partner.name)'''
            arg_list += tuple(branch)
        ...
        if not branch:
            ...
            query = '''SELECT l.id
                    ...
                    WHERE (l.account_id = account_account.id) AND (l.move_id = am.id)
                        ...
                        AND ((l.partner_id IN %s) OR (l.partner_id IS NULL))
                    AND (l.date <= %s)
                    AND am.branch_id IN %s
                    AND l.company_id IN %s'''
            cr.execute(query, (
            tuple(move_state), tuple(account_type), date_from, tuple(partner_ids), date_from, tuple(branch),
            tuple(company_ids))
                       )
        aml_ids = cr.fetchall()
    ...
    def _get_report_values(self, docids, data=None):
        ...
        movelines, total, dummy = self._get_partner_move_lines(account_type, date_from, target_move,
                                                               data['form']['period_length'], data['form']['branch'])
        return {
            'doc_ids': self.ids,
            'doc_model': model,
            'data': data['form'],
