...
class ScanWAQRCode(models.TransientModel):
    ...
    def _get_default_image(self):
        ...
        url = 'https://api.chat-api.com/instance' + Param.get('whatsapp_instance_id') + '/status?token=' + Param.get(
            'whatsapp_token')
        response = requests.get(url)
        json_response = json.loads(response.text)
        if (response.status_code == 201 or response.status_code == 200) and (
                json_response['accountStatus'] == 'got qr code'):
            # qr_code_image
            # qr_image = base64.b64encode(json_response['qrCode'])
            qr_code_url = 'https://api.chat-api.com/instance' + Param.get(
                'whatsapp_instance_id') + '/qr_code?token=' + Param.get('whatsapp_token')
...
class SendWAMessageResPartner(models.TransientModel):
    ...
    def action_send_msg_res_partner(self):
        ...
        DOMAIN = "https://app.rmss.io" #https://chatwoot.example.com
        API_ACCESS_TOKEN = "XvE3SZyfDigoELZY7aEf8zuN" #rkoo0op2PPsihsv8JW3IjfiF
        ACCOUNT_ID = '1' #1
        INBOX_ID = '1' #1

        chatwoot_client = ChatWoot(DOMAIN, API_ACCESS_TOKEN, ACCOUNT_ID, INBOX_ID)

        contacts = chatwoot_client.create_message("5","helo mashaal","outgoing",False)
        print(contacts)
        status_url = 'https://api.chat-api.com/instance' + Param.get(
                        ...
                        if self.attachment_ids:
                            for attachment in self.attachment_ids:
                                with open("/tmp/" + attachment.name, 'wb') as tmp:
                                    ...
                                    url_send_file = 'https://api.chat-api.com/instance' + Param.get(
                                        'whatsapp_instance_id') + '/sendFile?token=' + Param.get('whatsapp_token')
                                    headers_send_file = {
                                        "Content-Type": "application/json",
                                    }
                                    dict_send_file = {
...
class SendWAMessageSendResPartner(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if res_id:
            ...
            rec = self.env[active_model].browse(res_id)
            Attachment = self.env['ir.attachment']
            res_name = 'Invoice_' + rec.number.replace('/',
                                                       '_') if active_model == 'account.move' else rec.name.replace('/',
                                                                                                                    '_')
            msg = result.get('message', '')
            result['message'] = msg
    ...
    def action_send_msg_res_partner(self):
        ...
        chatwoot_client = ChatWoot(DOMAIN, API_ACCESS_TOKEN, ACCOUNT_ID, INBOX_ID)
        print(chatwoot_client.list_inboxes())
        contacts = chatwoot_client.search_contacts("tarik mouaatadil")
        print(contacts)
    ...
    def default_get(self, fields):
        ...
        if active_model == 'account.move':
            if rec.name:
                ...
                res_name = 'Invoice_' + rec.name.replace('/',
                                                         '_') if active_model == 'account.move' else rec.name.replace(
                    '/', '_')
            ...
            if active_model == 'sale.order':
                if rec.partner_id.mobile and rec.partner_id.country_id.phone_code:
                    ...
                    if self.env['ir.config_parameter'].sudo().get_param(
                            'pragmatic_odoo_whatsapp_integration.group_order_info_msg'):
                        msg += "\n\nYour "
            ...
            if active_model == 'stock.picking':
                if rec.partner_id.mobile and rec.partner_id.country_id.phone_code:
                    ...
                    if rec.partner_id.parent_id:
                        msg += "(" + rec.partner_id.parent_id.name + ")"
                    if self.env['ir.config_parameter'].sudo().get_param(
                            'pragmatic_odoo_whatsapp_integration.group_stock_info_msg'):
                        msg += "\n\nYour "
                        # if self.env.context.get('proforma'):
                        #     msg += "in attachment your pro-forma invoice"
                        # else:
                    ...
                    if self.env['ir.config_parameter'].sudo().get_param(
                        ...
                        for line_id in rec.move_ids_without_package:
                            msg += "\n\n*Product:* " + line_id.product_id.name + "\n*Qty:* " + str(
                                line_id.product_uom_qty) + " " + str(line_id.product_uom.name) + "\n*Done:* " + str(
            ...
            if active_model == 'purchase.order':
                if rec.partner_id.mobile and rec.partner_id.country_id.phone_code:
                    ...
                    if self.env['ir.config_parameter'].sudo().get_param(
                        ...
                        for line_id in rec.order_line:
                            msg += "\n\n*Product:* " + line_id.product_id.name + "\n*Qty:* " + str(
                                line_id.product_qty) + " " + str(line_id.product_uom.name) + "\n*Unit Price:* " + str(
                                line_id.price_unit) + "\n*Subtotal:* " + str(line_id.price_subtotal)
                            msg += "\n------------------"
                    msg += "\n Please find attached document which will help you to get detailed information."
