...
class AccountsHierarchy(models.TransientModel):
    ...
    def _get_html(self):
        ...
        result['html'] = self.env.ref('accounts_hierarchy.report_accounts_heirarchy')._render(rcontext)
        return result
    ...
    def get_lines(self, wizard_id=None, line_id=None, **kw):
        ...
        if wizard_id:            
            context.update({
                ...
                'state' : self.browse(wizard_id).target_move or '',
            })
        rec_id = False
        level = 1
        if kw:
    ...
    def accounts_hierarchy_open_wizard(self):        
        ...
        result_context = literal_eval(result.get('context',"{}")) or {}
    ...
    def get_child_ids(self, wizard_id=None, account_id=None):
        ...
        if wizard_id:
            context.update({
                ...
                'state' : self.browse(wizard_id).target_move,
            })        
        if wizard_id and account_id:
            ...
            if context['state'] == 'posted':
                result.append(('move_id.state', '=', 'posted'))
            else:
                result.append(('move_id.state', 'in', ['posted','draft']))
