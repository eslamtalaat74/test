...
class AccountMove(models.Model):
    ...
    @api.depends('amount_total', 'amount_tax', 'date_planned', 'partner_id')
    def generate_zatac_code(self):
        ...
        for record in self:
            qr_code_str = ''
            if record.date_planned and record.partner_id.vat:
                seller_name_enc = get_qr_encoding(1, record.partner_id.name)
                company_vat_enc = get_qr_encoding(2, record.partner_id.vat)
                time_sa = fields.Datetime.context_timestamp(self.with_context(tz='Asia/Riyadh'), record.date_planned)
