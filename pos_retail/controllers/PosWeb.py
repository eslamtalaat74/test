...
class pos_controller(PosController):
    ...
    def pos_web(self, config_id=False, **k):
        ...
        if (not pos_session and config_id) or k.get('switchPos'):
...
class web_login(Home):  # TODO: auto go directly POS when login
    ...
    def web_login(self, *args, **kw):
        ...
        if request.session.uid:
            ...
            if (pos_config or user.pos_config_ids) and user.pos_portal_user:
                return request.redirect('/web?#action=pos_retail.point_of_sale_portal')
        return response