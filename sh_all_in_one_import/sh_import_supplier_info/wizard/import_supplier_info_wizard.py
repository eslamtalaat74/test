...
class ImportSupplierInforWizard(models.TransientModel):
    ...
    product_model = fields.Selection([('pro_var', 'Product Variants'),
    ...
    def validate_field_many2many(self, field_name, field_ttype, field_value,
        ...
        if field_required and field_value in (None, ""):
            ...
            name_relational_model = self.env[
                'product.supplierinfo'].fields_get()[field_name]['relation']
                            ...
                            if row[0] not in (None,
                                ...
                                if search_product:
                                    ...
                                    if product_field_nm == 'product_id' and search_product.product_tmpl_id:
                                        vals.update({
                                            'product_tmpl_id':
                                            search_product.product_tmpl_id.id
                                        })
                                else:
                                    skipped_line_no[str(
                                ...
                                if search_vendor:
                                    ...
                                    skipped_line_no[str(
                                        counter
                                    )] = " - Vendor not found or is not a supplier. "
                                    counter = counter + 1
                                    continue
                ...
                try:
                    ...
                    for row in range(sheet.nrows):
                        try:
                            ...
                            if sheet.cell(
                                    row,
                                    0).value not in (None, "") and sheet.cell(
                                        row, 1).value not in (None, ""):
                                vals = {}
                                ...
                                search_product = product_obj.search([
                                    (field_nm, '=', sheet.cell(row, 0).value)
                                ],
                                                                    limit=1)
                                if search_product:
                                    vals.update(
