        ...
        if self and self.file:
            ...
            if self.import_type == 'csv':
                    ...
                    for row in myreader:
                        try:
                            ...
                            if row[2] not in ("", None):
                                type_id = self.env[
                                    'account.account.type'].sudo().search(
                                        [('name', '=', row[2])], limit=1)
                                if type_id:
                                    vals.update({'user_type_id': type_id.id})
                                else:
                                    skipped_line_no[str(
                                        counter)] = " - Type not found."
                                    counter = counter + 1
                                    continue
                            else:
                                skipped_line_no[str(
                            ...
                            if row[3] not in ("", None):
                                ...
                                vals.update({'tax_ids': [(6, 0, taxes_list)]})
                ...
                if counter > 1:
                    ...
                    res = self.show_success_msg(completed_account,
                                                skipped_line_no)
                    return res
                    ...
                    for row in range(sheet.nrows):
                        try:
                            ...
                            if sheet.cell(row, 7).value not in ("", None):
                                if sheet.cell(row, 7).value.strip() == 'FALSE':
                                    vals.update({'deprecated': False})
                                elif sheet.cell(row,
                                                7).value.strip() == 'TRUE':
                                    vals.update({'deprecated': True})
