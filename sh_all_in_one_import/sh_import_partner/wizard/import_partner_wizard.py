...
class ImportPartnerWizard(models.TransientModel):
    ...
    method = fields.Selection([('create', 'Create Customer/Vendor'),
    ...
    contact_update_by = fields.Selection([
        ...
        ('mobile', 'Mobile'),
                                         ...
                                         default='name',
                                         string="Customer/Vendor Update By",
                                         required=True)
                    ...
                    for row in myreader:
                        try:
                            ...
                            if row[1] != '':
                                ...
                                if row[13] != '' and row[0].strip(
                                ) != 'Company':
                                    search_title = self.env[
                                        ...
                                        try:
                                            with open(image_path,
                                                ...
                                                if image_base64:
                                                    ...
                                                    skipped_line_no[str(
                                                        counter
                                                    )] = " - Could not find the image or please make sure it is accessible to this user. "
                                                    counter = counter + 1
                                                    continue
                                        except Exception as e:
                                            skipped_line_no[str(
                                                counter
                                            )] = " - Could not find the image or please make sure it is accessible to this user " + ustr(
                                                e)
                                            counter = counter + 1
                                            continue
                                ...
                                elif self.contact_update_by == 'email' and row[
                                        11] not in ['', None]:
                                    partner_domain = [
                                        ('email', '=', row[11]),
                                    ]
                                elif self.contact_update_by == 'phone' and row[
                                        9] not in ['', None]:
                                    partner_domain = [
                                        ('phone', '=', row[9]),
                                    ]
                                elif self.contact_update_by == 'mobile' and row[
                                        10] not in ['', None]:
                                    partner_domain = [
                                        ('mobile', '=', row[10]),
                                    ]
                    ...
                    for row in range(sheet.nrows):
                        try:
                            ...
                            if sheet.cell(row, 1).value != '':
                                ...
                                if isinstance(sheet.cell(row, 6).value, float):
                                    zip_int = int(sheet.cell(row, 6).value)
                                    zip_str = str(zip_int)
                                    vals.update({'zip': zip_str})
                                else:
                                    vals.update(
                                        {'zip': sheet.cell(row, 6).value})
                                if isinstance(sheet.cell(row, 9).value, float):
                                    phone = int(sheet.cell(row, 9).value)
                                    phone_str = str(phone)
                                    vals.update({'phone': phone_str})
                                else:
                                    vals.update(
                                        {'phone': sheet.cell(row, 9).value})
                                if isinstance(
                                        sheet.cell(row, 10).value, float):
                                    mobile = int(sheet.cell(row, 10).value)
                                    mobile_str = str(mobile)
                                    vals.update({'mobile': mobile_str})
                                else:
                                    vals.update(
                                ...
                                elif self.contact_update_by == 'email' and sheet.cell(
                                        row, 11).value not in ['', None]:
                                    partner_domain = [
                                        ('email', '=', sheet.cell(row,
                                                                  11).value),
                                    ]
                                elif self.contact_update_by == 'phone' and sheet.cell(
                                        row, 9).value not in ['', None]:
                                    if isinstance(
                                            sheet.cell(row, 9).value, float):
                                        phone = int(sheet.cell(row, 9).value)
                                        phone_str = str(phone)
                                        partner_domain = [
                                            ('phone', '=', phone_str),
                                ...
                                elif self.contact_update_by == 'mobile' and sheet.cell(
                                        row, 10).value not in ['', None]:
                                    if isinstance(
                                            sheet.cell(row, 10).value, float):
                                        mobile = int(sheet.cell(row, 10).value)
                                        mobile_str = str(mobile)
