...
class ImportProductTemplateWizard(models.TransientModel):
                            ...
                            if row[0].strip() not in (None, ""):
                                ...
                                if row[9].strip() in (None, ""):
                                    ...
                                    if search_uom:
                                        ...
                                        skipped_line_no[str(
                                            counter
                                        )] = " - Unit of Measure - Unit(s) not found. "
                                        counter = counter + 1
                                        continue
                                else:
                                    search_uom = self.env['uom.uom'].search(
                                ...
                                if row[10].strip() in (None, ""):
                                    ...
                                    if search_uom_po:
                                        ...
                                        skipped_line_no[str(
                                            counter
                                        )] = " - Purchase Unit of Measure - Unit(s) not found. "
                                        counter = counter + 1
                                        continue
                                else:
                                    search_uom_po = self.env['uom.uom'].search(
                                ...
                                if created_product_tmpl and created_product_tmpl.product_variant_id and created_product_tmpl.type == 'product' and row[
                                        17] != '':
                                    stock_vals = {
                                        'product_tmpl_id':
                    ...
                    for row in range(sheet.nrows):
                        try:
                            ...
                            if sheet.cell(row,
                                ...
                                product_type = 'consu'
                                if sheet.cell(row,
                                              3).value.strip() == 'Service':
                                    product_type = 'service'
                                elif sheet.cell(row, 3).value.strip(
                                ) == 'Stockable Product':
                                    product_type = 'product'

                                categ_id = False
                                if sheet.cell(row,
                                              4).value.strip() in (None, ""):
                                    search_category = self.env[
                                        'product.category'].search(
                                            [('complete_name', '=', 'All')],
                                            limit=1)
                                ...
                                some_taxes_not_found = False
                                if sheet.cell(row,
                                              13).value.strip() not in (None,
                                                                        ""):
                                    for x in sheet.cell(row,
                                                        13).value.split(','):
                                        x = x.strip()
                                        if x != '':
                                            search_customer_tax = self.env[
                                ...
                                if some_taxes_not_found:
                                    ...
                                    continue
                                invoicing_policy = 'order'
                                if sheet.cell(row, 15).value.strip(
                                ) == 'Delivered quantities':
                                    invoicing_policy = 'delivery'

                                vals = {
                                    'name':
                                    sheet.cell(row, 0).value.strip(),
                                    'sale_ok':
                                    can_be_sold,
                                    'purchase_ok':
                                    can_be_purchased,
                                ...
                                if sheet.cell(row,
                                              18).value.strip() not in (None,
                                                                        ""):
                                    image_path = sheet.cell(row,
                                                            18).value.strip()
                                    if "http://" in image_path or "https://" in image_path:
                                        try:
                                            r = requests.get(image_path)
                                ...
                                if created_product_tmpl and created_product_tmpl.product_variant_id and created_product_tmpl.type == 'product' and sheet.cell(
                                        row, 17).value != '':
                                    stock_vals = {
                                        'product_tmpl_id':
                                        created_product_tmpl.id,
