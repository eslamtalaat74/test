...
class StockPicking(models.Model):
    ...
    def button_validate(self):
        ...
        for stock in self:
            template_id = self.env['account.sms.template'].search([('model_id.model','=', stock._name),
            ...
            if template_id.account_gateway == 'msg91':
                ...
                if stock.partner_id.mobile:
                    payload = "{ \"sender\": \"%s\", \"route\": \"%s\", \"country\": \"%s\", \"sms\": [ { \"message\": \"%s\", \"to\": [ \"%s\"] } ] }" %\
                                (msg_sender, msg_route, msg_country_code, template_body, stock.partner_id.mobile)
                ...
                if stock.partner_id.mobile:
                    ...
                    sms_message = SmsMessage(source="php", body=template_body, to=stock.partner_id.mobile)
                ...
                template_body = template_id.template_body
                if stock.partner_id.mobile:
                    sms_messages =  urllib.parse.urlencode({'apikey': textlocal_authkey, 'numbers': stock.partner_id.mobile,
